...
model = YOLO("yolov8n.pt") # Ensure this is a model trained for person detection
...
def is_falling(bbox):
    ...
    # More robust fall detection by considering changes in aspect ratio over time could be implemented here.
    if aspect_ratio < 0.7:  # Adjust this threshold as needed
        return True
    return False
...
def generate_frames():
    ...
    while True:
        ...
        results = model.track(frame, persist=True, classes=0) # Track only person class (class 0 for COCO)
        ...
        if results and results[0].boxes:
            for box in results[0].boxes:
                ...
                if conf > 0.5: #only detect if confidence is higher than 0.5
                    if is_falling(xyxy):
        ...
        ret, buffer = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 60]) # Lower JPEG quality for smaller frames
        if ret:
